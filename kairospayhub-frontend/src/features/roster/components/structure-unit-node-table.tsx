...
} from '@/shared/ui/structure-badges'
...
import { unitsPhoneCard } from '@/features/roster/lib/phone-cards'
...
export function StructureUnitNodeTable({
  ...
  return (
    ...
    >
      ...
      <PhoneList
        phone={phone}
        empty={emptyMessage ?? `No ${layer.displayName.toLowerCase()} in this unit yet. Use Add above.`}
        items={table.getRowModel().rows.map((row) => {
          ...
          return {
            id: row.original.id,
            ...unitsPhoneCard(row.original, childLayer?.displayName),
            actions: (
              <RosterUnitActionsMenu
                ...
                onChangeLeader={
                  policy.canChangeLeader && onChangeLeader
                    ? () => onChangeLeader(row.original)
                    : undefined
                }
                onEdit={policy.canRename ? () => onEdit(row.original) : undefined}
                onDelete={policy.canDelete ? () => onDelete(row.original) : undefined}
              />
            ),
          }
        })}
