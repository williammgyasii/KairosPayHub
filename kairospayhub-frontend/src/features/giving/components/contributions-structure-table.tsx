...
import type { Contribution, ContributionStatus, GivingProgram } from '@/features/giving/api'
...
import { ContributionStatusBadge } from '@/features/giving/components/giving-badges'
import { StructureContributionsUnitSheet } from '@/features/giving/components/structure-contributions-unit-sheet'
    ...
    <Card>
      ...
      <CardContent>
        ...
        {nodes.length === 0 ? (
          ...
          <PhoneList
            phone={phone}
            empty="Nothing at this level."
            onItemOpen={(id) => {
              const node = nodes.find((row) => row.id === id)
              if (node) onOpenOverview(node)
            }}
            items={nodes.map((node) => ({
              id: node.id,
              ...labeledPhoneCard(
                node.name,
                [node.layerLabel, formatAmount(node.totalAmount)],
                [
                  { label: 'Name', value: node.name },
                  { label: 'Level', value: node.layerLabel },
                  { label: 'Payments', value: node.paymentCount },
                  { label: 'Amount', value: formatAmount(node.totalAmount) },
                ],
              ),
            }))}
          >
        ...
        <PhoneList
          ...
          items={payments.map((payment) => ({
            ...
            ...labeledPhoneCard(
              ...
              [
                { label: 'Date sent', value: formatGivingDate(payment.dateSent) },
