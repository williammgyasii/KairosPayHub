...
import type { Contribution, ContributionListQuery, GivingProgram } from '@/features/giving/api'
import { formatAmount, listAllContributions, listProgramContributions } from '@/features/giving/api'
...
import { ContributionBulkBatchModal } from '@/features/giving/components/contribution-bulk-batch-modal'
import { ContributionDetailModal } from '@/features/giving/components/contribution-detail-modal'
import { RejectContributionModal } from '@/features/giving/components/reject-contribution-modal'
import { ProgramApprovalBadge, LegacyParentContributionBadge } from '@/features/giving/components/giving-badges'
import {
  ...
  return (
    <>
      <div className="overflow-hidden rounded-lg border border-border/60">
        ...
        <PhoneList
          phone={phone}
          empty="No contributions on this page."
          onItemOpen={(id) => {
            if (id.startsWith('batch-')) {
              const batchId = id.slice('batch-'.length)
              const entry = visibleDisplayRows.find(
                (row) => row.kind === 'batch' && row.batchId === batchId,
              )
              if (entry?.kind === 'batch') setBatchTarget(entry.contributions)
              return
            }
            const pending = visibleDisplayRows.find(
              (row) => row.kind === 'single' && row.contribution.id === id,
            )
            if (pending?.kind === 'single') {
              setViewTarget(pending.contribution)
              return
            }
            const approved = rows.find((row) => row.id === id)
            if (approved) setViewTarget(approved)
          }}
          items={
            mode === 'pending'
              ? visibleDisplayRows.map((entry) =>
                  entry.kind === 'batch'
                    ? {
                        id: `batch-${entry.batchId}`,
                        ...labeledPhoneCard(
                          `Batch · ${summarizeBatch(entry.batchId, entry.contributions).memberCount} members`,
                          [
                            formatAmount(summarizeBatch(entry.batchId, entry.contributions).totalAmount),
                            formatGivingDate(summarizeBatch(entry.batchId, entry.contributions).dateSent),
                          ],
                          [
                            { label: 'Members', value: summarizeBatch(entry.batchId, entry.contributions).memberCount },
                            { label: 'Amount', value: formatAmount(summarizeBatch(entry.batchId, entry.contributions).totalAmount) },
                            { label: 'Date', value: formatGivingDate(summarizeBatch(entry.batchId, entry.contributions).dateSent) },
                          ],
                        ),
                      }
                    : {
                        ...
                        ...labeledPhoneCard(
                          entry.contribution.memberName,
                          [
                            formatAmount(entry.contribution.amount, entry.contribution.currency),
                            formatGivingDate(entry.contribution.dateSent),
                          ],
                          [
                            { label: 'Member', value: entry.contribution.memberName },
                            { label: 'Amount', value: formatAmount(entry.contribution.amount, entry.contribution.currency) },
                            { label: 'Date', value: formatGivingDate(entry.contribution.dateSent) },
                            { label: 'Campaign', value: entry.contribution.programTitle },
                          ],
                        ),
                      },
                )
              : rows.map((row) => ({
                  ...
                  ...labeledPhoneCard(
                    row.memberName,
                    [formatAmount(row.amount, row.currency), formatGivingDate(row.dateSent)],
                    [
                      ...
                      { label: 'Campaign', value: row.programTitle },
                    ],
                  ),
                }))
          }
