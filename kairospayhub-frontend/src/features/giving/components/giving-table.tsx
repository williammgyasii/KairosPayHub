...
import type { GivingCampaignStats } from '@/features/giving/components/giving-metrics'
import {
  GivingCampaignActionsMenu,
  type CampaignAction,
} from '@/features/giving/components/giving-campaign-actions-menu'
import { givingTypeLabel, scopeKindLabel } from '@/features/giving/lib/giving-ui'
      ...
      <PhoneList
        ...
        items={table.getRowModel().rows.map((row) => ({
          ...
          ...labeledPhoneCard(
            ...
            [
              { label: 'Campaign', value: row.original.title },
              { label: 'Type', value: givingTypeLabel(row.original.givingType) },
              { label: 'Period', value: row.original.periodLabel },
              { label: 'Scope', value: scopeKindLabel(row.original.scopeKind) },
              { label: 'Status', value: row.original.status },
              {
                label: 'Approved',
                value:
                  showTotals && row.original.stats != null
                    ? formatAmount(row.original.stats.totalApprovedAmount)
                    : null,
              },
            ],
