...
import { formatAmount, listMemberGivingTotals } from '@/features/giving/api'
import type { StructureTree } from '@/api/structure'
import { MemberGivingBreakdownModal } from '@/features/giving/components/member-giving-breakdown-modal'
...
import { formatGivingDate } from '@/features/giving/lib/giving-ui'
import {
    ...
    <>
      <div className="min-w-0 max-w-full space-y-3">
        ...
        <div className="overflow-hidden rounded-xl border border-border/60 bg-card">
          ...
          <PhoneList
            phone={phone}
            empty="No member totals yet."
            onItemOpen={(id) => {
              const member = table.getRowModel().rows.find((row) => row.original.memberId === id)?.original
              if (member) setBreakdownMember(member)
            }}
            items={table.getRowModel().rows.map((row) => ({
              ...
              ...labeledPhoneCard(
                ...
                [
                  `#${row.original.rank} · ${formatAmount(row.original.approvedTotal)}`,
                  row.original.lastDateSent
                    ? formatGivingDate(row.original.lastDateSent)
                    : 'No gifts yet',
                ],
                [
                  { label: 'Rank', value: row.original.rank },
                  { label: 'Member', value: row.original.memberName },
                  { label: 'Approved', value: formatAmount(row.original.approvedTotal) },
                  { label: 'Gifts', value: row.original.approvedCount },
                  { label: 'Pending', value: row.original.pendingCount },
                  {
                    label: 'Last gift',
                    value: row.original.lastDateSent
                      ? formatGivingDate(row.original.lastDateSent)
                      : null,
                  },
                ],
