...
import type { Contribution } from '@/features/giving/api'
import { formatAmount } from '@/features/giving/api'
import type { StructureTree } from '@/api/structure'
import { groupContributionsByMember, type MemberGivingHistory } from '@/features/giving/lib/contribution-structure'
import { formatGivingDate } from '@/features/giving/lib/giving-ui'
import { MemberGivingHistorySheet } from '@/features/giving/components/member-giving-history-sheet'
          ...
          {members.length === 0 ? (
            ...
            <>
              <PhoneList
                ...
                onItemOpen={(id) => {
                  const member = pageMembers.find((row) => row.memberId === id)
                  if (member) setSelectedMember(member)
                }}
                items={pageMembers.map((member) => ({
                  id: member.memberId,
                  ...labeledPhoneCard(
                    member.memberName,
                    [member.structurePath, formatAmount(member.approvedTotal)],
                    [
                      { label: 'Member', value: member.memberName },
                      { label: 'Structure', value: member.structurePath },
                      { label: 'Payments', value: member.contributionCount },
                      { label: 'Approved', value: formatAmount(member.approvedTotal) },
                      {
                        label: 'Last given',
                        value: member.lastDateSent ? formatGivingDate(member.lastDateSent) : null,
                      },
                    ],
                  ),
                }))}
