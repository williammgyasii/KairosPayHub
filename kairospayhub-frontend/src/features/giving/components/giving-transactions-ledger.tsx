...
import { formatAmount, listAllContributions } from '@/features/giving/api'
import type { StructureTree } from '@/api/structure'
import { enrichContribution } from '@/features/giving/lib/contribution-structure'
import {
...
} from '@/features/giving/components/giving-badges'
      ...
      <div className="overflow-hidden rounded-xl border border-border/60">
        ...
        <PhoneList
          ...
          onItemOpen={(id) => {
            ...
            if (row) setViewTarget(row)
          }}
          items={rows.map((row) => ({
            id: row.id,
            ...labeledPhoneCard(
              row.memberName,
              [formatAmount(row.amount, row.currency), row.programTitle],
              [
                ...
                { label: 'Date sent', value: formatGivingDate(row.dateSent) },
