#!/usr/bin/env python3
"""Create a dev login for a cell leader (Powerhouse church)."""
...
DEFAULT_CELL_NODE_ID = "93f6f837-e455-4398-864d-b74c46425330"  # Zion Cell 1
...
def main() -> int:
    ...
    parser.add_argument("--email", default="zion.cell1@powerhouse.dev")
    parser.add_argument("--name", default="David Boateng")
    parser.add_argument("--password", default="DevPass123!")
    parser.add_argument("--cell-node-id", default=DEFAULT_CELL_NODE_ID)
    ...
    with psycopg.connect(conn_str) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                """,
                (args.cell_node_id, POWERHOUSE_CHURCH_ID),
            )
            row = cur.fetchone()
            if not row:
                raise SystemExit(f"Cell node not found: {args.cell_node_id}")
            _, cell_name, leader_member_id = row
            ...
            if leader_member_id is None:
                ...
                cur.execute(
                    ...
                    (leader_member_id, args.cell_node_id),
                )
            else:
                cur.execute(
                    """
                    UPDATE church_members
                    SET "Email" = %s, "Name" = %s, "Position" = 'CellLeader', "AuthUserId" = %s
                    WHERE "Id" = %s
                    """,
            ...
            cur.execute(
                ...
                VALUES
                    (%s, %s, %s, 'CellLeader', %s, %s)
                """,
                (uuid.uuid4(), POWERHOUSE_CHURCH_ID, auth_user_id, args.cell_node_id, now),
            )

        conn.commit()

    print(f"Cell leader login ready for {cell_name}:")
    print(f"  Email:    {args.email}")
    print(f"  Password: {args.password}")
    print("Sign out, then log in at http://127.0.0.1:5173/login")
    print("Open Attendance → Submissions to enter roll call.")
    return 0
