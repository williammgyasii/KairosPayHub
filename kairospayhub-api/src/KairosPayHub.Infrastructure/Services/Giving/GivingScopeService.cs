...
public class GivingScopeService(KairosDbContext db)
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (program.ScopeKind is ProgramScopeKind.FellowshipGroup or ProgramScopeKind.UnitGroup)
        {
    ...
    public async Task<(ProgramScopeKind Kind, Guid? ScopeNodeId, IReadOnlyList<Guid>? ScopeNodeIds)> ResolveProgramScopeAsync(
        Guid churchId,
        string? scopeKindRaw,
        Guid? scopeNodeId,
        IReadOnlyList<Guid>? scopeNodeIds,
        CancellationToken ct = default)
    {
        var distinctIds = (scopeNodeIds ?? [])
            .Concat(scopeNodeId is null ? [] : new[] { scopeNodeId.Value })
            .Distinct()
            .ToList();

        if (distinctIds.Count == 0)
        {
            if (string.IsNullOrWhiteSpace(scopeKindRaw)
                || scopeKindRaw.Equals("ChurchWide", StringComparison.OrdinalIgnoreCase))
            {
                return (ProgramScopeKind.ChurchWide, null, null);
            }

            if (!Enum.TryParse<ProgramScopeKind>(scopeKindRaw, ignoreCase: true, out var legacyKind))
                throw new BadRequestException($"Unknown scope kind: {scopeKindRaw}");
            if (legacyKind != ProgramScopeKind.ChurchWide)
                throw new BadRequestException("Scope node is required for scoped programs");
            return (ProgramScopeKind.ChurchWide, null, null);
        }

        var nodes = await db.StructureNodes.AsNoTracking()
            .Where(n => n.ChurchId == churchId && distinctIds.Contains(n.Id))
            .Select(n => new { n.Id, n.LayerId })
            .ToListAsync(ct);
        if (nodes.Count != distinctIds.Count)
            throw new BadRequestException("Scope node not found");

        var layerIds = nodes.Select(n => n.LayerId).Distinct().ToList();
        if (layerIds.Count != 1)
            throw new BadRequestException("Multi-select scope units must be on the same layer");

        var layerType = await db.StructureLayers.AsNoTracking()
            .Where(l => l.Id == layerIds[0])
            .Select(l => (StructureLayerType?)l.StandardType)
            .SingleOrDefaultAsync(ct)
            ?? throw new BadRequestException("Scope layer not found");

        if (distinctIds.Count > 1)
        {
            var groupKind = layerType == StructureLayerType.Fellowship
                ? ProgramScopeKind.FellowshipGroup
                : ProgramScopeKind.UnitGroup;
            return (groupKind, null, distinctIds);
        }

        var singleId = distinctIds[0];
        var singleKind = layerType switch
        {
            StructureLayerType.PFCC => ProgramScopeKind.PFCC,
            StructureLayerType.Fellowship => ProgramScopeKind.Fellowship,
            _ => ProgramScopeKind.Unit,
        };
        return (singleKind, singleId, null);
    }

    public bool CanCreateGivingPrograms(Actor actor) =>
